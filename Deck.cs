...
[PunRPC]
    ...
    private IEnumerator DelayedDistributeCards()
    {
        ...
        yield return new WaitForSeconds(2.0f); // Adjust the delay for the turn as needed
        photonView.RPC("DealTurnCardRPC", RpcTarget.AllViaServer);

        // Wait some time before dealing the river
        yield return new WaitForSeconds(2.0f); // Adjust the delay for the river as needed
