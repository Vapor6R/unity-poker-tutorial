...
public class Deck : MonoBehaviourPunCallbacks
{
    ...
    private PlayerCardHandler playerHandler;
...
{
    // Clear the list of card components
    cards.Clear();
    
    // Find the DeckParent GameObject
    
    
    GameObject[] cardsz = GameObject.FindGameObjectsWithTag("Card");

        // Loop through each card and destroy it
        foreach (GameObject card in cardsz)
        {
            // Check if the object is a PhotonView and if we have authority to destroy it
    ...
    //Debug.Log("All cards cleared and destroyed.");
}
    ...
    public void RestartRPC()
    {
       photonView.RPC("ClearCommunityCards", RpcTarget.MasterClient);
		...
		photonView.RPC("ClearCards", RpcTarget.AllBuffered);
       ...
       StartCoroutine(ShuffleDeckAfterDelay());
	    StartCoroutine(DelayedDistributeCards());
			...
			StartCoroutine(GameManager.Instance.Logo());
...
private void AddCommunityCardRPC(int cardViewID, int positionIndex)
{
    ...
    if (cardView != null)
    {
        ...
        if (card != null && communityCardsParent != null)
        {
            ...
            float spacing = -110f; // Adjust the spacing value as needed
