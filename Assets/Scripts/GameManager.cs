...
public void RPC_PassTurnToNextBySeat(int currentSeat)
{
    ...
    if (activeP.Count == 0)
    {
        photonView.RPC("ResetTurnStatesForOthers", RpcTarget.AllBuffered);
		foreach (var pm in FindObjectsOfType<PlayerManager>())
 if (pm != null){pm.UI.SetActive(false);}
        StartCoroutine(WaitAndShow());
        return;
    }
    ...
    // ✅ Find the next active player AFTER currentSeat (circular search)
    // Start from currentSeat + 1 and wrap around if needed
    PlayerManager nextPlayer = null;
    int startingSeat = currentSeat;
    int maxSeats = FindObjectsOfType<PlayerManager>().Max(p => p.seatIndex) + 1;
    
    // Search forward from currentSeat + 1
    for (int offset = 1; offset <= maxSeats; offset++)
    {
        int checkSeat = (currentSeat + offset) % maxSeats;
        nextPlayer = activeP.FirstOrDefault(p => p.seatIndex == checkSeat);
    ...
    if (nextPlayer == null)
    {
        Debug.LogWarning($"⚠️ Could not find next player after seat {currentSeat}. Using first active player.");
        nextPlayer = activeP[0];
    }
    
    // ✅ Check if we've cycled back to the starting player
    int playersWithChips = activeP.Count(p => p.chipCount > 0);
    
    if (nextPlayer.seatIndex == startingSeat)
    {
        if (playersWithChips <= 1)
        {
            Debug.Log(
                $"✅ Turn would return to same seat ({startingSeat}) with only {playersWithChips} player(s) with chips. " +
        ...
        {
            Debug.Log($"⚠️ Turn cycled back to seat {startingSeat} but {playersWithChips} players still have chips. Continuing.");
        }
    }
    
    Debug.Log($"✅ Passing turn from seat {currentSeat} → {nextPlayer.PlayerName} (seat {nextPlayer.seatIndex})");
